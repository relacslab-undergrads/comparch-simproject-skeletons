...
/* } */
        if (readq.size() == 0 && writeq.size() == 0) {
          write_mode = false;
        } else if (readq.size() == 0) {
          write_mode = true;
        } else if (writeq.size() == 0) {
          write_mode = false;
        } else {
          auto wreq = scheduler->get_head(writeq.q);
          assert(wreq != writeq.q.end());

          auto rreq = scheduler->get_head(readq.q);
          assert(rreq != readq.q.end());

          if (rreq->arrive <= wreq->arrive) write_mode = false;
          else write_mode = true;
        }
